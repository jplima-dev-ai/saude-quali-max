#!/usr/bin/env python3
"""Regression contracts for the safe cleanup in release 3.8.8."""
...
require(config.get("versao") == routes.get("version") == package.get("version") == "3.8.8", "versões divergentes")
require("qualimax-v3.8.8" in worker, "cache desatualizado")
...
require("## [3.8.8]" in (ROOT / "docs/CHANGELOG.md").read_text(encoding="utf-8"), "changelog ausente")
...
if errors:
    print("v3.8.8 FAILED\n" + "\n".join(errors))
    sys.exit(1)
print("v3.8.8 OK: redundâncias removidas, gerador limpo e 81 páginas preservadas")
